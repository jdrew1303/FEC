...
GenericGF.prototype = {
        init : function (primitive, size, b) {
                ...
                this.expTable = new Buffer(size);
                this.logTable = new Buffer(size);
                ...
                this.zero = new GenericGFPoly(this, new Buffer([0]));
                this.one = new GenericGFPoly(this, new Buffer([1]));
        },
        ...
        buildMonomial : function (degree, coefficient) {
                ...
                var coefficients = new Buffer(degree + 1);
                coefficients.fill(0);
...
GenericGFPoly.prototype = {
        ...
        addOrSubtract : function (other) {
                ...
                var sumDiff = new Buffer(largerCoefficients.length);
                var lengthDiff = largerCoefficients.length - smallerCoefficients.length;
                // Copy high-order terms only found in higher-degree polynomial's coefficients
                largerCoefficients.copy(sumDiff, 0, 0, lengthDiff);
        ...
        multiplyGenericGFPoly : function (other) {
                ...
                var product = new Buffer(aLength + bLength - 1);
                product.fill(0);
        ...
        multiplyByMonomial : function (degree, coefficient) {
                ...
                var size = this.coefficients.length;
                var product = new Buffer(size + degree);
                product.fill(0);
                for (var i = 0; i < size; i++) {
...
ReedSolomonEncoder.prototype = {
        init : function (field) {
                ...
                this.cachedGenerators.push(new GenericGFPoly(field, new Buffer([1])));
        },
        ...
        buildGenerator : function (degree) {
                if (degree >= this.cachedGenerators.length) {
                        ...
                        for (var d = this.cachedGenerators.length; d <= degree; d++) {
                                var nextGenerator = lastGenerator.multiply(new GenericGFPoly(this.field, new Buffer([ 1, this.field.exp(d - 1 + this.field.generatorBase) ]) ));
        ...
        encode : function (toEncode, ecBytes) {
                ...
                var infoCoefficients = new Buffer(dataBytes);
                toEncode.copy(infoCoefficients, 0, 0, dataBytes);
                ...
                coefficients.copy(toEncode, dataBytes + numZeroCoefficients, 0, coefficients.length);
        }
};
...
ReedSolomonDecoder.prototype = {
        ...
        decode : function (received, twoS) {
                ...
                var syndromeCoefficients = new Buffer(twoS);
        ...
        findErrorLocations : function (errorLocator) {
                ...
                if (numErrors == 1) { // shortcut
                        return new Buffer([  errorLocator.getCoefficient(1) ]);
                }
                var result = new Buffer(numErrors);
                result.fill(0);
                var e = 0;
